...
> 브라우저에서 발음을 녹음하고 AI 분석 결과를 비교하며 반복 훈련하는 발음 개선 서비스

2025년 10월부터 11월까지 진행한 SSAFY 팀 프로젝트입니다. 사용자는 발성, 단어, 문장 훈련을 수행하고 연습 기록과 피드백을 확인할 수 있습니다. 저는 프런트엔드 개발을 맡아 발성 훈련과 브라우저 녹음 처리, API 구조 개선을 담당했습니다.

![돋음 메인 화면](./exec/시연시나리오/메인화면.png)

## 주요 기능

- **발성 훈련**: MPT, 점점 크게, 점점 작게, 크게-작게, 작게-크게 등 5가지 훈련 제공
- **단어·문장 훈련**: 카메라와 마이크로 발음을 녹화하고 기준 발음과 비교
- **분석 피드백**: Praat 음성 지표와 AI 분석 결과를 항목별로 제공
...
팀원과 구현 범위에 대한 의견이 달랐을 때 요구사항과 일정을 함께 검토하고, 제 판단이 과도했음을 인정해 그래프 영상 대신 이미지 캡처를 사용하기로 합의했습니다. 선택 기능을 줄이고 서버가 요구한 핵심 데이터에 집중해 발성 훈련 5종의 전체 흐름을 완성했습니다.
...
| Infra | Docker Compose, Nginx, Jenkins, AWS EC2, Google Cloud VM·Storage |

## 로컬 실행

프런트엔드 화면은 다음 명령으로 실행할 수 있습니다.
...
기본 API 주소는 `http://localhost:8000/api/v1`입니다. 녹음·분석·기록 저장까지 확인하려면 FastAPI, PostgreSQL, 저장소와 GPU 추론 서버 설정이 추가로 필요합니다. 전체 배포 환경은 [`exec/MANUAL.md`](./exec/MANUAL.md)에 정리돼 있습니다.
...
- 프런트엔드 프로덕션 빌드와 ESLint로 정적 검증합니다.
- 카메라·마이크 권한과 녹음 형식은 Chromium 계열 브라우저를 기준으로 구현했습니다.
- GPU 모델과 외부 API가 필요한 전체 분석 환경은 로컬 프런트엔드 실행만으로 재현되지 않습니다.
- 자동화된 프런트엔드 테스트는 추가하지 못했으며, 주요 사용자 흐름은 팀 시연 시나리오로 확인했습니다.
...
이 저장소는 팀 프로젝트 결과를 GitHub에 복제한 공개본입니다. 전체 팀 코드와 개인 기여 코드를 함께 포함하며, 위 커밋 링크에서 제가 담당한 변경을 확인할 수 있습니다.
